#!/usr/bin/env python3
"""
hl_trader_screen_rev2.py
Hyperliquid trader screen - stage 1 (data collection only, no scoring).

rev2: adds HL_MODE = winners (rev1 behaviour, output data/hyperliquid/) | losers | random
      (losers/random write to data/hyperliquid/<mode>/ so earlier data is never overwritten).
  losers : still-funded, recently active accounts with the biggest all-time losses
           (allTime PnL <= -$50k, account value >= $20k, month volume >= $1M, all-time volume >= $10M,
            volume/account <= 3000), sorted by all-time PnL ascending.
  random : same activity filters, no PnL condition, random sample (fixed seed) = control group.
...
MODE = (os.environ.get("HL_MODE", "") or "winners").strip().lower()
if MODE not in ("winners", "losers", "random"):
    raise SystemExit(f"Unknown HL_MODE {MODE!r}")
OUT_DIR = os.environ.get("HL_OUT_DIR", "data/hyperliquid" if MODE == "winners" else f"data/hyperliquid/{MODE}")
...
# losers / random mode filters
MAX_ALLTIME_PNL_LOSERS = _env_float("HL_MAX_ALLTIME_PNL_LOSERS", -50_000)
MIN_ACCOUNT_VALUE_ACTIVE = _env_float("HL_MIN_ACCOUNT_VALUE_ACTIVE", 20_000)
MIN_MONTH_VLM = _env_float("HL_MIN_MONTH_VLM", 1_000_000)
MIN_ALLTIME_VLM = _env_float("HL_MIN_ALLTIME_VLM", 10_000_000)
...
def shortlist(rows):
    if MODE == "winners":
        return shortlist_winners(rows)
    keep = []
    for r in rows:
        av, ap, mv, vlm = r["account_value"], r["allTime_pnl"], r["month_vlm"], r["allTime_vlm"]
        if None in (av, ap, mv, vlm) or av < MIN_ACCOUNT_VALUE_ACTIVE:
            continue
        if mv < MIN_MONTH_VLM or vlm < MIN_ALLTIME_VLM or vlm / av > MAX_VLM_TO_ACCT:
            continue
        if MODE == "losers" and ap > MAX_ALLTIME_PNL_LOSERS:
            continue
        keep.append(dict(r, vlm_to_acct=round(vlm / av, 1)))
    if MODE == "losers":
        keep.sort(key=lambda r: r["allTime_pnl"])
    else:
        import random
        keep.sort(key=lambda r: r["address"])
        random.Random(RANDOM_SEED).shuffle(keep)
    return keep[:SHORTLIST_SIZE]


def shortlist_winners(rows):
    keep = []
    for r in rows:
...
def main():
    ...
    log = {"run_utc": run_ts, "mode": MODE, "settings": {
